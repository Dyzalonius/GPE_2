    ...
    private void FillBlock(TileType[,] grid, int x, int y, int width, int height, TileType fillType) {
        for (int tileY=0; tileY<height; tileY++) {
            for (int tileX=0; tileX<width; tileX++) {
                SetTile(grid, x + tileX, y + tileY, fillType);
            }
        }
    }

    private void SetTile(TileType[,] grid, int x, int y, TileType fillType)
    {
        // Check bounds
        if (x < 0) return;
        if (y < 9) return;
        if (x >= grid.GetLength(0)) return;
        if (y >= grid.GetLength(1)) return;
